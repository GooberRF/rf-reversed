...
types:
  ...
  vertex:
    seq:
      ...
      - id: lightmap_coords
        type: uv
        if: _parent.as<face>.surface_index != -1
  ...
  face:
    seq:
      ...
      - id: surface_index
        type: s4
        doc: index in geometry::surfaces array or -1 if face does not use lightmap
      - id: face_id
        type: s4
  ...
  geometry:
    seq:
      - id: unknown1
        type: u4
        if: _root.header.version >= 0xC8
        doc: typically zero, unused by the game engine
      - id: modifiability
        type: u4
        if: _root.header.version >= 0xC8
        doc: typically zero, related to GeoMod
      - id: name
        type: vstring
        doc: typically empty, unused by the game engine
      - id: modifiability_old
        type: u4
        if: _root.header.version < 0xC8
        doc: typically zero, related to GeoMod
      - id: num_textures
  ...
  portal:
    seq:
      ...
      - id: point2
        type: vec3
  surface:
    doc: lightmap information, can be shared by multiple faces
    seq:
      - id: lightmap_index
        type: s4
      ...
      - id: should_smooth
        type: s4
        doc: 1 if face belongs to any smoothing group, 0 otherwise
      - id: unknown_zero
        type: s4
        doc: typically zero
      ...
      - id: u_coefficient
        type: s4
        doc: which position component is used as lightmap U coordinate (x - 0, y - 1, z - 2), used in lighting calculation
      - id: v_coefficient
        type: s4
        doc: which position component is used as lightmap V coordinate (x - 0, y - 1, z - 2), used in lighting calculation
      - id: uv_add
        type: uv
        doc: added to lightmap UVs when calculating lighting
      - id: uv_scale
        type: uv
        doc: scales lightmap UVs when calculating lighting
      - id: room_index
        type: s4
